...
export async function startAgentProcess(name: string, opts: { fresh?: boolean } = {}): Promise<{ ok: boolean; pid?: number; error?: string }> {
  ...
  try {
    ...
    // provider bug, 2026-08-05). own_team agents are excluded too (see above).
    ...
    // gated on needsFleetOauth. own_team agents (needsFleetOauth excludes them)
    // still need isolation for their own /login credential, hence `|| isOwnTeam`.
