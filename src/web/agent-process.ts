...
import { agentDir, listAgentNames, readAgentModel, readAgentClaudeConfigDir, readAgentClaudePlan, readAgentChannelProvider, readAgentAuthMode, readAgentDisplayName, readAgentRemoteConfig, readAgentRemoteHost, readAgentRunAsUser, readAgentMemoryIsolation, readAgentCustomProvider } from './agent-config.js'
...
): { provider: ProviderKind; exportsStr: string; customApiKey?: string | null } {
  if (customProviderDef) {
    let headerExport: string
    ...
    return {
      provider: 'custom',
      exportsStr: `export ANTHROPIC_BASE_URL="${customProviderDef.baseUrl}" && ${headerExport}export ANTHROPIC_MODEL='${model}' && `,
      customApiKey,
    }
  }
...
export async function startAgentProcess(name: string, opts: { fresh?: boolean } = {}): Promise<{ ok: boolean; pid?: number; error?: string }> {
  ...
  try {
    ...
    // isCustom takes priority over built-in providers; customProviderDef carries the
    // endpoint/auth info. resolveProviderEnv does the early-return for custom providers.
    const isClaude = !isCustom && model.startsWith('claude-')
    // When authHeader=x-api-key, the key is exported as ANTHROPIC_API_KEY.
    // The CLI shows an interactive approval prompt for that env var, which
    // cannot be answered in --channels mode. We pre-seed the approval into
    ...
    const { exportsStr: providerEnv, customApiKey: customApiKeyForApproval = null } = resolveProviderEnv(model, getSecret, customProviderDef ?? undefined)
    ...
    // The CLAUDE_CONFIG_DIR isolation (ensureIsolatedChannelConfigDir) still
    // applies to BYO-endpoint channel agents -- it prevents plugin-slot
    // collision regardless of provider type. Only the OAuth token export is
    // gated on needsFleetOauth.
    ...
    const cmd = `${umaskPrefix}export PATH="/opt/homebrew/bin:$HOME/.bun/bin:/usr/local/bin:/usr/bin:/bin:$PATH" && ${unsetTokens} && ${autoUpdaterEnv}${byoUnsetEnv}${promptSuggestionEnv}${mcpEnv}${channelSetup}${apiKeyEnv}${claudeConfigEnv}${oauthTokenEnv}${providerEnv}cd "${dir}" && ${claudeBin()} ${continueFlag}${skipFlag}--model ${shSingleQuote(model)} ${channelFlag}`.trimEnd()
